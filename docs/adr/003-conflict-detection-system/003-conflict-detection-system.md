...
`my-time` 프로젝트는 로컬과 서버 간의 데이터 동기화에서 발생하는 충돌을 정확하게 감지하고 해결하기 위해, 단순한 타임스탬프 비교 대신 Git과 유사한 Linked List of Hashes 방식을 채택했습니다.
...
`my-time`은 사용자의 시간 로그 데이터를 다루는 만큼, 데이터 보존이 가장 중요한 가치입니다.
